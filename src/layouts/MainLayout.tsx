...
export function MainLayout() {
  ...
  return <div className={`app-shell${collapsed ? ' sidebar-collapsed' : ''}`}>
    ...
    <div className="workspace"><header className="header">
      ...
      <div className="header-account"><div className="unit"><Building2 size={18} aria-hidden="true" /><div><small>Unidade</small><strong>{contexto?.unidade_nome || contexto?.nome_fantasia || 'Não vinculada'}</strong></div></div><Link className="user-profile" to="/configuracoes" aria-label={`Usuário ${displayName} — configurações`}><span className="avatar">{initials}</span><span><small>Usuário</small><strong>{displayName}</strong></span></Link><button className="icon-button" type="button" aria-label="Sair" title="Sair" onClick={() => void signOut()}><LogOut size={18} /></button></div>
