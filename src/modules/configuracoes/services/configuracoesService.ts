...
function validateName(value: string, label: string) {
  if (value.trim().length < 2) throw new Error(`${label} deve possuir pelo menos 2 caracteres.`)
}
...
function mutationError(message?: string) {
  ...
  const known = [
    ...
    'Unidade fora do escopo autorizado', 'Perfil fora do escopo autorizado',
    'Perfil de sistema ou fora do escopo nao pode ser alterado', 'Perfil possui usuarios ativos vinculados',
    'Perfil inativo nao pode ser atribuido', 'Perfil ja atribuido ao usuario', 'Perfil nao esta atribuido ao usuario',
    'Nenhuma alteracao de status identificada', 'Nenhuma alteracao de unidade identificada',
    'Convite pendente nao encontrado', 'Selecione ao menos um perfil', 'Perfil invalido, inativo ou fora da empresa',
    'Razao social invalida', 'Nome da unidade invalido', 'Nome do setor invalido', 'Nome do perfil invalido', 'Permissao invalida',
  ]
...
export function createConfiguracoesService(client: SupabaseClient) {
  ...
  async function authorizeMutation(scope: ReadScope, permission: string) {
    return authorize(scope, permission, new AbortController().signal)
  }
  ...
  return {
    async usuarios(scope: ReadScope, signal: AbortSignal): Promise<UsuariosData> {
      ...
      const [usuarios, empresas, unidades, perfis] = await Promise.all([
        ...
        readAll<Empresa>((from, to) => client.from('empresas')
          .select('id,nome_fantasia,razao_social,cnpj,ativo', { count: 'exact' }).eq('id', scope.empresaId)
          .order('id').range(from, to).abortSignal(signal).returns<Empresa[]>(), signal),
        readAll<Unidade>((from, to) => client.from('unidades')
          .select('id,empresa_id,nome,codigo,ativo', { count: 'exact' }).eq('empresa_id', scope.empresaId)
    ...
    async estrutura(scope: ReadScope, signal: AbortSignal): Promise<EstruturaData> {
      ...
      const [empresas, unidades, setores] = await Promise.all([
        readAll<Empresa>((from, to) => client.from('empresas').select('id,nome_fantasia,razao_social,cnpj,ativo', { count: 'exact' })
          .eq('id', scope.empresaId).range(from, to).abortSignal(signal).returns<Empresa[]>(), signal),
        readAll<Unidade>((from, to) => client.from('unidades').select('id,empresa_id,nome,codigo,ativo', { count: 'exact' })
          .eq('empresa_id', scope.empresaId).order('nome').range(from, to).abortSignal(signal).returns<Unidade[]>(), signal),
        readAll<Setor>((from, to) => client.from('setores').select('id,unidade_id,nome,codigo,ativo', { count: 'exact' })
          .order('nome').range(from, to).abortSignal(signal).returns<Setor[]>(), signal),
      ])
      if (!empresas[0]) throw new Error('A empresa da sua sessão não foi encontrada.')
      const unitIds = new Set(unidades.map(item => item.id))
      return { empresa: empresas[0], unidades, setores: setores.filter(item => unitIds.has(item.unidade_id)) }
    },
    ...
    async auditoria(scope: ReadScope, signal: AbortSignal): Promise<AuditoriaData> {
      ...
      const eventos = await readAll<AuditoriaEvento>((from, to) => client.from('auditoria_eventos')
        .select('id,empresa_id,unidade_id,ator_id,entidade,registro_id,acao,antes,depois,justificativa,contexto,created_at', { count: 'exact' })
        .eq('empresa_id', scope.empresaId).order('created_at', { ascending: false }).range(from, to).abortSignal(signal).returns<AuditoriaEvento[]>(), signal)
      const usuarios = hasPermission(context, 'usuarios.gerenciar')
        ? await readAll<Pick<Usuario, 'id' | 'nome' | 'email'>>((from, to) => client.from('usuarios')
          .select('id,nome,email', { count: 'exact' }).eq('empresa_id', scope.empresaId).order('id')
          .range(from, to).abortSignal(signal).returns<Pick<Usuario, 'id' | 'nome' | 'email'>[]>(), signal)
        : []
    ...
    async alterarUnidade(scope: ReadScope, usuarioId: string, unidadeId: string | null, justificativa: string) {
      validateUuid(usuarioId, 'Usuário'); if (unidadeId) validateUuid(unidadeId, 'Unidade'); validateJustification(justificativa); await authorizeMutation(scope, 'usuarios.gerenciar')
      const { error } = await client.rpc('admin_usuario_alterar_unidade', { p_usuario_id: usuarioId, p_unidade_id: unidadeId, p_justificativa: justificativa.trim() })
      if (error) throw new Error(mutationError(error.message))
    },
    async alterarPerfil(scope: ReadScope, usuarioId: string, perfilId: string, acao: PerfilAction, justificativa: string) {
      validateUuid(usuarioId, 'Usuário'); validateUuid(perfilId, 'Perfil'); validateJustification(justificativa); await authorizeMutation(scope, 'perfis.gerenciar')
      const { error } = await client.rpc('admin_usuario_alterar_perfil', { p_usuario_id: usuarioId, p_perfil_id: perfilId, p_acao: acao, p_justificativa: justificativa.trim() })
      if (error) throw new Error(mutationError(error.message))
    },
    ...
    async convidarUsuario(scope: ReadScope, input: { email: string; nome: string; unidadeId: string | null; perfilIds: string[]; justificativa: string }) {
      ...
      validateName(input.nome, 'Nome'); if (input.unidadeId) validateUuid(input.unidadeId, 'Unidade')
    ...
    async perfilCriar(scope: ReadScope, nome: string, descricao: string, permissaoIds: string[], justificativa: string) {
      validateName(nome, 'Nome do perfil'); permissaoIds.forEach(id => validateUuid(id, 'Permissão')); validateJustification(justificativa); await authorizeMutation(scope, 'perfis.gerenciar')
      const { data, error } = await client.rpc('admin_perfil_criar', { p_nome: nome.trim(), p_descricao: descricao.trim(), p_permissao_ids: permissaoIds, p_justificativa: justificativa.trim() })
      if (error) throw new Error(mutationError(error.message)); return data as string
    },
    async perfilAtualizar(scope: ReadScope, perfilId: string, nome: string, descricao: string, ativo: boolean, permissaoIds: string[], justificativa: string) {
      validateUuid(perfilId, 'Perfil'); validateName(nome, 'Nome do perfil'); permissaoIds.forEach(id => validateUuid(id, 'Permissão')); validateJustification(justificativa); await authorizeMutation(scope, 'perfis.gerenciar')
      const { error } = await client.rpc('admin_perfil_atualizar', { p_perfil_id: perfilId, p_nome: nome.trim(), p_descricao: descricao.trim(), p_ativo: ativo, p_permissao_ids: permissaoIds, p_justificativa: justificativa.trim() })
      if (error) throw new Error(mutationError(error.message))
    },
    async empresaAtualizar(scope: ReadScope, razaoSocial: string, nomeFantasia: string, cnpj: string, justificativa: string) {
      validateName(razaoSocial, 'Razão social'); validateJustification(justificativa); await authorizeMutation(scope, 'estrutura.gerenciar')
      const { error } = await client.rpc('admin_empresa_atualizar', { p_razao_social: razaoSocial.trim(), p_nome_fantasia: nomeFantasia.trim(), p_cnpj: cnpj.trim(), p_justificativa: justificativa.trim() })
      if (error) throw new Error(mutationError(error.message))
    },
    async unidadeSalvar(scope: ReadScope, unidadeId: string | null, nome: string, codigo: string, ativo: boolean, justificativa: string) {
      if (unidadeId) validateUuid(unidadeId, 'Unidade'); validateName(nome, 'Nome da unidade'); validateJustification(justificativa); await authorizeMutation(scope, 'estrutura.gerenciar')
      const { data, error } = await client.rpc('admin_unidade_salvar', { p_unidade_id: unidadeId, p_nome: nome.trim(), p_codigo: codigo.trim(), p_ativo: ativo, p_justificativa: justificativa.trim() })
      if (error) throw new Error(mutationError(error.message)); return data as string
    },
    async setorSalvar(scope: ReadScope, setorId: string | null, unidadeId: string, nome: string, codigo: string, ativo: boolean, justificativa: string) {
      if (setorId) validateUuid(setorId, 'Setor'); validateUuid(unidadeId, 'Unidade'); validateName(nome, 'Nome do setor'); validateJustification(justificativa); await authorizeMutation(scope, 'estrutura.gerenciar')
      const { data, error } = await client.rpc('admin_setor_salvar', { p_setor_id: setorId, p_unidade_id: unidadeId, p_nome: nome.trim(), p_codigo: codigo.trim(), p_ativo: ativo, p_justificativa: justificativa.trim() })
      if (error) throw new Error(mutationError(error.message)); return data as string
    },
  }
}
