...
import type { Empresa, Setor, Unidade } from '../types'
import { DetailsDialog } from './DetailsDialog'
import { QueryFeedback } from './QueryFeedback'

function CompanyEditor({ empresa, onSaved }: { empresa: Empresa; onSaved: () => void }) {
  ...
  const save = async () => {
    const result = await actions.empresaAtualizar(razao, fantasia, cnpj, reason)
    if (result.ok) { setReason(''); onSaved() }
  }
  return <section className="settings-admin-card"><div><h3>Empresa</h3><p>Dados cadastrais utilizados no contexto organizacional.</p></div>{(actions.error || actions.success) && <p className={`settings-notice${actions.error ? ' warning' : ''}`} role={actions.error ? 'alert' : 'status'}>{actions.error || actions.success}</p>}<div className="settings-fields"><label>Razão social<input value={razao} onChange={event => setRazao(event.target.value)} /></label><label>Nome fantasia<input value={fantasia} onChange={event => setFantasia(event.target.value)} /></label><label>CNPJ<input value={cnpj} onChange={event => setCnpj(event.target.value)} placeholder="Opcional" /></label><label>Justificativa<input value={reason} onChange={event => setReason(event.target.value)} placeholder="Motivo da alteração" /></label></div><button className="button primary" disabled={razao.trim().length < 2 || reason.trim().length < 5 || actions.busy !== null} onClick={() => void save()}>Salvar empresa</button></section>
}
...
export function EstruturaPanel() {
  ...
  return <>
    ...
    <CompanyEditor empresa={empresa} onSaved={query.reload} />
