...
test('visibilidade não concede administração, nem por URL direta', async ({ page }) => {
  ...
  await expect(page.getByRole('navigation', { name: 'Seções de configurações' }).getByRole('button')).toHaveCount(2)
...
test('checagem compartilhada nega contexto inativo, bloqueado, sem empresa e sem permissão', () => {
  ...
  for (const override of [{ ativo: false }, { status: 'pendente' as const }, { status: 'bloqueado' as const }, { empresa_id: null }, { permissoes: [] }]) expect(hasPermission({ ...context, ...override }, 'usuarios.gerenciar')).toBe(false)
...
test('serviço nega contexto revogado antes de consultar tabelas administrativas', async () => {
  const paths: string[] = []
  const client = createClient('https://bpf-test.supabase.co', 'test-key', { auth: { persistSession: false }, global: { fetch: async input => { paths.push(String(input)); return new Response(JSON.stringify([{ ...context, permissoes: ['configuracoes.visualizar'] }]), { headers: { 'content-type': 'application/json' } }) } } })
...
for (const scenario of [
  ...
  test(`serviço recusa ${scenario.name} antes da leitura administrativa`, async () => {
    const paths: string[] = []
    const client = createClient('https://bpf-test.supabase.co', 'test-key', { auth: { persistSession: false }, global: { fetch: async input => { paths.push(String(input)); return new Response(JSON.stringify([{ ...context, ...scenario.override }]), { headers: { 'content-type': 'application/json' } }) } } })
    ...
    if (scenario.scope?.empresaId && scenario.scope.empresaId !== companyId) expect(paths.length).toBeGreaterThanOrEqual(1)
    else { expect(paths).toHaveLength(1); expect(paths[0]).toContain('v_meu_contexto') }
  })
}

test('serviço envia mutação somente após revalidar a permissão', async () => {
  const paths: { url: string; method: string; body: string | null }[] = []
  const client = createClient('https://bpf-test.supabase.co', 'test-key', { auth: { persistSession: false }, global: { fetch: async (input, init) => { const url=String(input);paths.push({url,method:init?.method??'GET',body:typeof init?.body==='string'?init.body:null});if(url.includes('v_meu_contexto'))return new Response(JSON.stringify([context]),{headers:{'content-type':'application/json'}});return new Response('null',{headers:{'content-type':'application/json'}}) } } })
  ...
  expect(paths).toHaveLength(2); expect(paths[0].url).toContain('v_meu_contexto'); expect(paths[1].url).toContain('/rpc/admin_usuario_alterar_status'); expect(paths[1].method).toBe('POST'); expect(paths[1].body).toContain(colleagueId)
})
...
test('serviço pagina respostas acima do limite do servidor', async () => {
  ...
  const client = createClient('https://bpf-test.supabase.co', 'test-key', { auth: { persistSession: false }, global: { fetch: async input => { const url=new URL(String(input));const table=url.pathname.split('/').pop();const all=table==='v_meu_contexto'?[context]:table==='permissoes'?permissions:[];const from=Number(url.searchParams.get('offset')??0);if(table==='permissoes')offsets.push(from);return new Response(JSON.stringify(all.slice(from,from+500)),{headers:{'content-type':'application/json','content-range':`0-0/${all.length}`}}) } } })
