...
Estado em 18/09/2026: `/configuracoes` oferece Visão Geral, Usuários e Perfis e Permissões, com acesso granular e consultas reais. A fundação de leitura administrativa está aplicada no Supabase remoto e determina a empresa pelo ator autenticado, com isolamento por RLS e perfis ativos compatíveis. A fundação de escrita administrativa também está aplicada e validada no remoto: alterações de status, unidade e vínculos de perfil são expostas apenas por comandos RPC auditados, com role restrita sem login/BYPASSRLS, sem UPDATE/INSERT/DELETE direto para `authenticated`, proteção contra autoelevação e contra remoção do último administrador efetivo. A tabela `auditoria_eventos` está protegida por RLS e não é acessível diretamente pelo cliente. Usuários `pendente` continuam fora desse fluxo e exigem onboarding dedicado. A próxima etapa é conectar esses comandos validados ao frontend de `Configurações → Usuários`, mantendo justificativa obrigatória e confirmações explícitas. Ver [módulo existente](CONFIGURACOES_USUARIOS_PERFIS.md), [leitura administrativa](LEITURA_ADMINISTRATIVA_USUARIOS.md) e [escrita administrativa e auditoria](ADMINISTRACAO_USUARIOS_ESCRITA.md).
